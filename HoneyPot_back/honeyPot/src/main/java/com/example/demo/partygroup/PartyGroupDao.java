...
public interface PartyGroupDao extends JpaRepository<PartyGroup, Integer> {
	...
	@Query(value = "update party_group set start_check=2 where board_num=:board_num and start_check = 1 or 4", nativeQuery = true)
	...
	@Query(value = "update party_group set start_check=4 where board_num=:board_num",  nativeQuery = true)
	void updateStartCheckTo4(@Param("board_num") int board_num);
	
	// 0 (구독 시작전)인 해당 boardNum을 가진 행을 삭제
	@Query(value = "delete from PartyGroup where board_Num = :board_num and start_check=0", nativeQuery = true)
