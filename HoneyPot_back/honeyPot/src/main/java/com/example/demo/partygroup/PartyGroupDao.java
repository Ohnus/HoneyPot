...
public interface PartyGroupDao extends JpaRepository<PartyGroup, Integer> {
	...
	@Transactional
	@Modifying
	@Query(value = "select * from party_group where board_num=:board_num and user_num=:user_num", nativeQuery = true)
	List<PartyGroup> checkPartyGroup(@Param("board_num") int board_num, @Param("user_num") String user_num);
