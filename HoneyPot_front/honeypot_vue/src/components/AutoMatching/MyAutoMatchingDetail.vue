...
export default {
    ...
    data() {
        return {
            boardNum: '',
            matchingNum: this.$route.query.matchingNum,
    ...
    created: function () {
        ...
        console.log(self.matchingNum)

        self.$axios.get('http://localhost:8988/partygroup/automatching/' + self.matchingNum )
            .then(function (res) {
                if (res.status == 200) {
                    self.partygroupdto = res.data.dto
                    self.boardNum = res.data.dto.boardNum.boardNum
                }
