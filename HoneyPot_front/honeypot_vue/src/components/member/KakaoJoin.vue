...
<!-- 로그인은 성공했고, 로그인하고 나서 본인인증 후, 본인인증한 정보 + 네이버 정보를 가져와야함
본인인증 정보는 이름과 핸드폰번호
네이버 정보는 이메일
새로 입력할 것은 닉네임
약관도 추가 필요 -->
...
<div v-else-if="certified" class="NaverJoinForm">    
...
export default {
    ...
    data() {
        return {
            ...
            authCodeMsgBlank: true,     // 공란 여부 확인
    ...
    methods: {
            ...
            self.$axios
                ...
                .then(function (res) {
                    if (res.status == 200) {
                     ...
                     self.id = res.data.userInfo.id.toString();
                     console.log(self.id);

                    self.$axios.get('http://localhost:8988/members/userNumCheck/' + 'KKO' + self.id)
                    .then(function (res) {
                        if (res.status == 200) {
                            if (res.data.flag) {    // 회원가입필요함
                                console.log(res.data.flag);
                            } else {
                                console.log("회원가입 되어있어. 로그인 진행시켜" + res.data.flag) // 이미 회원가입된 사람. 로그인 진행시켜
        ...
        join() {
            ...
            if (this.checkEmpty(self.email) && this.checkEmpty(self.nickname) && self.nicknameValid) {
                ...
                formdata.append('snsType', 1);
                formdata.append('userNum',self.id);
