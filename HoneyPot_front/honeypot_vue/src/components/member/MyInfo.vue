...
<div style="position: fixed; top:560px; left:800px; z-index: 2;">
...
<div class="myInfo" style="z-index: 1;">
    ...
    <div class="col-md-4">
    <div class="userImg">
    <img id="profile" :src="previewImage">

    <div style="margin-top:10px">
    <label for="file-input" class="custom-file-upload"><span>파일 선택</span></label>
    <input id="file-input" type="file" style="display: none;" @change="changeImg" accept="image/*">
...
export default {
    ...
    data() {
        return {
            ...
            name: '',
            phone: '',
            originalEmail: self.email,
            email: '',
            enterCode: '',              // 이메일 인증번호 입력값
            originalPwd: self.pwd,
            pwd: '',
            originalNickname: self.nickname,
            nickname: '',
            cash: ''
        }
    },
    ...
    methods: {
        ...
        profileImg() {
            if (this.profile == null) {
                this.previewImage = require('@/assets/images/BasicUserImg.png');
            } else {
                this.previewImage = 'http://localhost:8988/members/imgs/' + this.userNum;
            }

            this.profile = this.previewImage;
        },
        ...
        setDefaultProfile() {   // 기본 이미지로 설정 + 기존 이미지 삭제함
            this.previewImage = require('@/assets/images/BasicUserImg.png');
            this.delProfile();
        },
        ...
        delProfile(){   // 이미지 삭제
            ...
            self.$axios.delete('http://localhost:8988/members/imgs/'+ self.userNum)
            .then(function(res){
                if(res.status == 200){
                    if(res.data.flag){
                        self.profile = null;     //이미지경로 null로 설정하여 기본 이미지로 변경
                        self.uploadImage = null; //업로드된 이미지 초기화
                        location.reload();
                    }
                } else{
                    alert('에러코드:' + res.status)
        ...
        editInfo() {
            ...
            if (self.email == self.originalEmail) {                 // 변경된 내용 없을 경우
                formdata.append('email', self.originalEmail);       // 기존 값으로 적용
            } else if (self.email !== self.originalEmail) {         // 변경된 내용 있을 경우
                if (self.email !== '' && !self.authCodeValid) {     // 유효성 체크
                    alert('이메일 인증번호가 유효하지 않습니다.');
                    return;                                         // 수정 불가능한 상태면 종료
                } else {                                            // 유효성 체크 된 경우만 새 데이터로 업데이트
                    formdata.append('email', self.email);
                }
            }

            if (self.pwd == self.originalPwd) { // 
                formdata.append('pwd', self.originalPwd);
            } else if (self.pwd !== self.originalPwd) {
                if (self.pwd !== '' && !self.pwdCheckValid) {
            ...
            if (self.nickname == self.originalNickname) {
                formdata.append('nickname', self.originalNickname);
            } else if (self.nickname !== self.originalNickname) {
                if (self.nickname !== '' && !self.nicknameValid) {
        ...
        userOut() {
            ...
            if(answer) {
                self.$axios.delete('http://localhost:8988/members/' + self.userNum, {headers: {'token':token}} )
                .then(function (res) {
                    if (res.status == 200) {
                        console.log(res.data.flag + " / " + res.data.partyRemain + " / " + res.data.partyRemain);

                        if (res.data.flag) {
                            if (res.data.partyRemain && !res.data.cashRemain) {
                                alert("파티 참여 중인 상태에는 탈퇴가 불가능합니다.")
                            } else if (!res.data.partyRemain && res.data.cashRemain) {
                                alert("남아있는 캐시가 있어 탈퇴가 불가능합니다.")
                            } else if (res.data.partyRemain && res.data.cashRemain) {
                                alert("파티 참여 중이며 남아있는 캐시가 있어 탈퇴가 불가능합니다.")
                            } else if (!res.data.partyRemain && !res.data.cashRemain) {
                                self.delProfile();
                                alert('탈퇴가 완료되었습니다.')
                                self.logout();
                            ...
                            alert('탈퇴 진행 중 에러 발생. 고객센터로 문의해주세요.')
                        }
                    } else {
                        alert('에러코드: ' + res.status)
                    }
