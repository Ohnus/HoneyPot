...
export default {
    ...
    methods: {
        getToken() {
            ...
            .then(function (res) {
                if (res.status == 200) {
                    ...
                    self.$axios.get('http://localhost:8988/members/emailCheck', { params: { email: self.form.email } })
                    .then(function (res) {
                        if (res.status == 200) {
        ...
        join() {
            ...
            if (this.checkEmpty(self.form.email) && this.checkEmpty(self.nickname) && self.nicknameValid) {
